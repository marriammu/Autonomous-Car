...
def GetImageFromMobile():
    # IPAddress='http://192.168.43.170'
    IPAddress='http://192.168.43.105'
    # IPAddress='http://192.168.43.74'
    PortNumber=':8080'
    ImagePath='/shot.jpg'
...
def DetectSegments(HalfImage):
    ...
    Angle = np.pi/180
    MinimumThreshold = 10
    LineSegmets = cv2.HoughLinesP(HalfImage, rho= Rho, theta=Angle, threshold=MinimumThreshold, lines=np.array([]), minLineLength=8, maxLineGap=4)
    return LineSegmets
...
def MakePoints(Image, Line):
    ...
    MiddleOfTheImage = int(BottomOfTheImage / 2)  
...
def AverageSlopeIntercept(Image, LineSegmets):
    ...
    boundary = 1/2
    leftRegionBoundary =int(ColumnsNumber * (1 - boundary))
    # print(leftRegionBoundary)
    rightRegionBoundary =int( ColumnsNumber * boundary)
    # print(rightRegionBoundary)
    ...
    # print(rightFitAverage)
    # print(leftFitAverage)
    if len(leftFit) > 0:
...
def LinesDisplaying(Image, lines, lineColor = (0,255,0), lineWidth = 5):
